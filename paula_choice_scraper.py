...
# ============================================================
# 🧪 TEST MODE — đặt False khi muốn chạy FULL
TEST_MODE = False
TEST_LIMIT = 100       # số ingredient tối đa khi test
# ============================================================
...
def extract_text(obj) -> str:
    ...
    if isinstance(obj, dict):
        inner = obj.get("text", "")
        return extract_text(inner)  # recursive — vì text value có thể là list
...
class PaulaChoiceFullSpider(Spider):
    ...
    async def parse(self, response: Response):
        ...
        # ▼▼▼ TEST MODE: giới hạn số ingredient — xóa dòng slicing khi chạy full ▼▼▼
        items_to_process = ingredients[:TEST_LIMIT] if TEST_MODE else ingredients
        # ▲▲▲ -------------------------------------------------------------------▲▲▲
        ...
        # ▼▼▼ TEST MODE: bỏ qua phân trang khi test — xóa điều kiện `not TEST_MODE` khi chạy full ▼▼▼
        paging = d.get("paging", {})
        total = paging.get("total", 0)
        current_start = paging.get("start", 0)

        if not TEST_MODE and current_start == 0 and total > 2000:
        # ▲▲▲ -------------------------------------------------------------------▲▲▲
            base = (
...
if __name__ == "__main__":
    mode_label = f"TEST ({TEST_LIMIT} ingredients)" if TEST_MODE else "FULL"
    print(f"🚀 Starting Paula's Choice Scraper — mode: {mode_label}")
